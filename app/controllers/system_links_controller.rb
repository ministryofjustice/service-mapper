class SystemLinksController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @system_link.save
        ...
        format.json { render action: 'show', status: :created, location: @system_link }
      else
  ...
  def destroy
    @system_link.destroy
    respond_to do |format|
      format.html { redirect_to system_links_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def system_link_params
      params.require(:system_link).permit(:system_a_id, :system_b_id, :status, :envelope, :name, :description)
    end
end
