class StoryStage < ActiveRecord::Base
  ...
  def graph_json
    {:source => "#{self.from_type}_#{self.from_id}", :target => "#{self.to_type}_#{self.to_id}", :type => 'story_stage', :id => self.id, :url => Rails.application.routes.url_helpers.story_story_stage_path(self.story, self)}
  end
end
